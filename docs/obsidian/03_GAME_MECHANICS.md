...
## 7. Случайные события (Random Encounters)
При каждом броске кубика (D6) существует **15% шанс** срабатывания случайного события на дороге:

| Событие | Шанс | Эффект |
|---|---|---|
| **🚓 Экипаж ГАИ** | 50% от 15% | Штраф за превышение: **$100 – $300** |
| **🍀 Удачная находка** | 50% от 15% | Нашли кошелек/заначку: **+$50 – +$200** |

Механика добавляет азарта на пустых клетках и заставляет игрока всегда иметь небольшой запас наличности.

---

## 8. Механика Гонки (Переработка)
Гонка — это азартный поединок между игроками. Доступна только если в гараже есть **хотя бы 1 автомобиль**.

### 8.1. Дуэль в мультиплеере
1.  **Вызов**: Попадая на клетку `race`, игрок выбирает соперника из списка активных в комнате.
2.  **Ставка**: Инициатор предлагает ставку. Соперник может принять её или отказаться.
3.  **Участники**: Другие игроки могут «присоединиться» к заезду, внеся аналогичную ставку.
4.  **Расчет**: Победитель определяется на основе **Health** и **Тира** выбранного авто (у инициатора +1 к броску кубика как бонус клетки).

### 8.2. Solo гонка (vs бот «Боря»)
Временно упрощена: фиксированная ставка $500, быстрый расчет шансов на основе характеристик машины.
...
## 11. Новости рынка
Срабатывают каждые **5 ходов** (суммарных в комнате). Это обеспечивает высокую динамику цен.

- **База событий**: 10+ сценариев (Топливный кризис, Налог на роскошь, Японский дрифт и т.д.).
- **Визуал**: В Маркете и Гараже отображается активное событие с иконкой и описанием.

---

## 12. Соло-режим (Phase 4 — В РАЗРАБОТКЕ)
На текущем этапе приоритет отдан **наполнению контентом** и стабильности мультиплеера. Соло-механики (Бот-перехватчик, сюжетные квесты, рост долга) будут реализованы позже.
